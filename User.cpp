...
void User::printPurchasedStockList() {
   ...
   for(int i=0; i < (int) purchasedStocks.size(); i++){
       ...
       cout<< "STOCK NUMBER-"<<i+1<<endl<<endl;
        ...
        cout<<"Today's Starting Price: "<<purchasedStocks.at(i).getInitialPrice()<<endl;
        ...
        cout<<"Net Profit/Loss on this Stock: "<<purchasedStocks.at(i).getStockNetValue()<<endl;
...
//for adding a stock, overloaded operators
User& operator += (User& user,Industry st){
    
    st.setBoughtPrice(st.getCurrentPrice()); //when a stock is bought , the bought price is set
...
//for selling a stock, overloaded operators
 User& operator -= (User& user,Industry st){
     for(int i=0;i< (int) user.purchasedStocks.size();i++){
        ...
        //adjusting values for total value and net value of portfolio each time a stock is sold
        user.totalValue = 0;        
        user.netValue=0;
...
void User::printSoldStockList(){
    ...
    cout<<"The list of sold stocks contain: "<<endl<<endl;

    for(int i=0; i<(int)this->soldStocks.size();i++){
        cout<< "STOCK NUMBER-"<<i+1<<endl<<endl;
       
        // cout<<"Description of Stock: ";
        // soldStocks.at(i).printDescription();
        cout<<"Stock Name: "<<soldStocks.at(i).getName()<<endl;
        ...
        cout<<"Net Profit/Loss on this Stock: "<<soldStocks.at(i).getStockNetValue()<<endl;
